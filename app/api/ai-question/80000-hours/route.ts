...
const PODCAST_RSS = 'https://feeds.transistor.fm/80000-hours-podcast';
...
async function logCron(
  ...
  try {
    ...
    await (supabase as any).from('cron_logs').insert({
      job_name: 'ai-question-80000-hours',
      status,
      message,
      metadata,
    });
...
async function fetchLatestEpisode(): Promise<PodcastEpisode> {
  console.log('Fetching 80,000 Hours Podcast RSS feed...');
...
export async function POST(request: Request) {
  ...
  try {
    await logCron(supabase, 'started', '80,000 Hours Podcast question generation triggered');
    ...
    const prompt = `Generate a yes/no poll question based on this episode from the 80,000 Hours Podcast (a podcast about how to use your career to solve the world's most pressing problems, covering AI safety, global health, animal welfare, biosecurity, and effective altruism).
...
- Extract an interesting, debatable angle about career impact, effective altruism, AI safety, global priorities, or ethical decision-making
- Make it accessible and relevant to a general audience interested in making a positive impact with their careers
...
Reply with ONLY the question.`;
    ...
    if (LLM_PROVIDER === 'anthropic') {
      ...
      console.log('80,000 Hours Podcast question generation token usage:', {
    ...
    await logTokenUsage(
      supabase,
      'ai-question-80000-hours-embedding',
      EMBEDDING_MODEL,
    ...
    await logCron(supabase, 'success', `Published 80,000 Hours Podcast question: "${question.substring(0, 60)}..."`, {
    ...
    console.error('Error in 80,000 Hours Podcast question generation:', error);
