...
// Google News RSS for Elon Musk posts/statements
const NEWS_RSS_URL = 'https://news.google.com/rss/search?q=elon+musk+tweet+OR+post+OR+said+OR+X&hl=en-US&gl=US&ceid=US:en';
...
async function logCron(
  ...
  try {
    ...
    await (supabase as any).from('cron_logs').insert({
      job_name: 'ai-question-elon-musk',
      status,
      message,
      metadata,
    });
...
/**
 * Fetch yesterday's top news articles about Elon Musk from Google News RSS
 */
async function fetchYesterdaysNews(): Promise<NewsArticle[]> {
  console.log('Fetching Elon Musk news from Google News RSS...');
...
export async function POST(request: Request) {
  ...
  try {
    await logCron(supabase, 'started', 'Elon Musk news question generation triggered');
    ...
    const prompt = `Generate a yes/no poll question based on Elon Musk's recent activity as reported in the news.
...
- Focus on what Elon Musk said, posted, or did — extract a debatable angle
...
Reply with ONLY the question.`;
    ...
    await logTokenUsage(
      supabase,
      'ai-question-elon-musk-embedding',
      EMBEDDING_MODEL,
    ...
    await logCron(supabase, 'success', `Published Elon Musk question: "${question.substring(0, 60)}..."`, {
    ...
    console.error('Error in Elon Musk question generation:', error);
