...
const LESSWRONG_GRAPHQL = 'https://www.lesswrong.com/graphql';
...
async function logCron(
  ...
  try {
    ...
    await (supabase as any).from('cron_logs').insert({
      job_name: 'ai-question-lesswrong',
      status,
      message,
      metadata,
    });
...
/**
 * Fetch the top LessWrong post from yesterday (most upvotes) with body content
 */
async function fetchTopLessWrongPost(): Promise<{ post: LessWrongPost; source: string; totalPosts: number }> {
  const now = new Date();
  ...
  console.log(`Fetching LessWrong posts for ${afterDate} to ${beforeDate}...`);
  
  // Same GraphQL schema as EA Forum (both use Vulcan.js/ForumMagnum)
  const query = `
  ...
  try {
    response = await fetch(LESSWRONG_GRAPHQL, {
      method: 'POST',
      headers: {
    ...
    console.error('Network error fetching LessWrong:', fetchError);
    throw new Error(`Network error: ${fetchError instanceof Error ? fetchError.message : 'Unknown'}`);
  }
  
  console.log(`LessWrong response status: ${response.status}`);
  
  if (!response.ok) {
    const text = await response.text();
    console.error(`LessWrong error response: ${text.substring(0, 500)}`);
  ...
  console.log(`LessWrong returned ${allPosts.length} posts from API`);
  ...
  if (posts.length === 0) {
    ...
    throw new Error(`No LessWrong posts found for ${afterDate}`);
  }
  ...
  const topPost: LessWrongPost = {
    title: topPostData.title,
    url: `https://www.lesswrong.com/posts/${topPostData._id}/${topPostData.slug}`,
    slug: topPostData.slug,
    body: topPostData.contents?.markdown?.substring(0, 6000),
...
export async function POST(request: Request) {
  ...
  try {
    await logCron(supabase, 'started', 'LessWrong question generation triggered');
    ...
    // Fetch the top LessWrong post from yesterday
    let post: LessWrongPost;
    let source: string;
    let totalPosts: number;
    
    try {
      const result = await fetchTopLessWrongPost();
      post = result.post;
      source = result.source;
      totalPosts = result.totalPosts;
    } catch (fetchError) {
      const errorMessage = fetchError instanceof Error ? fetchError.message : 'Unknown fetch error';
      await logCron(supabase, 'error', `Failed to fetch LessWrong: ${errorMessage}`);
    ...
    const prompt = `Generate a yes/no poll question based on this LessWrong post.
...
Reply with ONLY the question.`;
    ...
    if (LLM_PROVIDER === 'anthropic') {
      ...
      console.log('LessWrong question generation token usage:', {
    ...
    console.log(`Generated LessWrong question: "${question}"`);
    ...
    await logTokenUsage(
      supabase,
      'ai-question-lesswrong-embedding',
      EMBEDDING_MODEL,
    ...
    await logCron(supabase, 'success', `Published LessWrong question: "${question.substring(0, 60)}..."`, {
    ...
    console.error('Error in LessWrong question generation:', error);
