...
async function logCron(
  ...
  try {
    ...
    await (supabase as any).from('cron_logs').insert({
      job_name: 'ai-question-doom-debates',
      status,
      message,
      metadata,
    });
...
async function fetchLatestEpisode(): Promise<PodcastEpisode> {
  console.log('Fetching Doom Debates Podcast RSS feed...');

  const response = await fetch(DOOM_DEBATES_RSS, {
    headers: {
...
export async function POST(request: Request) {
  ...
  try {
    await logCron(supabase, 'started', 'Doom Debates Podcast question generation triggered');
    ...
    const prompt = `Generate a yes/no poll question based on this episode from Doom Debates (a podcast where Liron Shapira debates AI safety and existential risk with guests, exploring whether AI will cause human extinction).
...
Reply with ONLY the question.`;
    ...
    if (LLM_PROVIDER === 'anthropic') {
      ...
      console.log('Doom Debates Podcast question generation token usage:', {
    ...
    await logTokenUsage(
      supabase,
      'ai-question-doom-debates-embedding',
      EMBEDDING_MODEL,
    ...
    await logCron(supabase, 'success', `Published Doom Debates Podcast question: "${question.substring(0, 60)}..."`, {
    ...
    console.error('Error in Doom Debates Podcast question generation:', error);
